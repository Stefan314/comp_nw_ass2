...
std::string createIPHeader(const std::string& checksum, const std::string& src_ip, const std::string& dest_ip, int flag) {
...
//    Flags = 0, because doesn't matter here
    ip_header += std::bitset<3>(flag).to_string();
...
//                        The msg that needs to be sent
                        std::string special_msg = createIPHeader(checksum, chngd_src, dest_ip, 0) +
...
//                        Used to get the source port number
                        ...
                        int src_port = ntohs(sin.sin_port);
//                        The msg that needs to be sent
                        std::string special_msg = createIPHeader(checksum, chngd_src, dest_ip, 4) +
                                                  createUDPHeader(src_port, port_nr, checksum);
//                        Creation of the buffer
                        char buff_special_msg[1400];
                        strcpy(buff_special_msg, special_msg.c_str());
